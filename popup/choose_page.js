...
function saveSET(wallpaper_foldername,wallpaper_filename){
  chrome.storage.sync.set({wallpaper_foldername:wallpaper_foldername,wallpaper_filename:wallpaper_filename}, function() {});
...
document.addEventListener("click", function(e) {
  ...
  if(e.target.id == "settings_submit"){
    ...
    //var wallpaper_filename = document.getElementById("wallpaper_filename").value;
    var sle = document.getElementById("select");
    var wallpaper_filename = sle.options[sle.selectedIndex].value;
    var wallpaper_foldername = document.getElementById("wallpaper_foldername").value;
    
      chrome.storage.sync.get(['wallpaper_foldername','wallpaper_filename'], function(result) {
        if(result.wallpaper_filename == undefined && result.wallpaper_foldername == undefined){
          //alert('Need Folder and File Name');          
          if(wallpaper_foldername == ''){
            alert('Need Folder Name');
          }
          if(wallpaper_filename == ''){
            alert('Need File Name');
          }
          if(wallpaper_foldername != '' && wallpaper_filename != ''){
            saveSET(wallpaper_foldername,wallpaper_filename);
          }

        }else if(result.wallpaper_foldername == undefined){
          if(wallpaper_foldername == ''){
            alert('Need Folder Name');
          }else{
            //document.getElementById("wallpaper_filename").value = result.wallpaper_filename;
            saveSET(wallpaper_foldername,wallpaper_filename);
          }          
        }else if(result.wallpaper_filename == undefined){
          //alert('Need File Name');
          if(wallpaper_filename == ''){
            alert('Need File Name');
          }else{
            document.getElementById("wallpaper_foldername").value = result.wallpaper_foldername;
            saveSET(wallpaper_foldername,wallpaper_filename);
          }
          
        }else if(wallpaper_foldername != '' && wallpaper_filename != ''){
          saveSET(wallpaper_foldername,wallpaper_filename);
        }
    ...
    chrome.storage.sync.get(['arrlist'], function(result) {
      var arrayG = result.arrlist;

      var select_div = document.getElementById("select");
      
        for (let cat of arrayG) {
